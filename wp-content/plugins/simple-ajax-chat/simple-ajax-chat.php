...
function sac_require_wp_version() {
	...
	global $wp_version, $sac_plugin, $sac_wp_vers;
	...
	if (isset($_GET['activate']) && $_GET['activate'] === 'true') {
		...
		if (version_compare($wp_version, $sac_wp_vers, '<')) {
			...
			if (is_plugin_active($plugin)) {
				...
				$msg  = '<strong>'. $sac_plugin .'</strong> '. __('requires WordPress ', 'sac');
				$msg .= $sac_wp_vers . __(' or higher, and has been deactivated! ', 'sac');
				$msg .= __('Please return to the', 'sac') .' <a href="'. admin_url('update-core.php') .'">';
				$msg .= __('WordPress Admin area', 'sac') .'</a> '. __('to upgrade WordPress and try again.', 'sac');
	...
	if (isset($_GET['activate']) && $_GET['activate'] === 'true') {
		...
		if ($check_table !== $table_name) {
			...
			$wpdb->insert($table_name, array(
				...
				'name' => __('The Admin', 'sac'), 
				'text' => __('High five! You&rsquo;ve successfully installed Simple Ajax Chat.', 'sac'), 
				'url'  => get_home_url(),
			...
			for ($row = 0; $row < 1; $row++) {
				...
				if (isset($query[$row]) && !empty($query[$row]) && is_array($query[$row])) {
					...
					while (list($key, $value) = each($query[$row])) {
						...
						$loop = $id .'---'. $name .'---'. $text .'---'. $time .' '. __('ago', 'sac') .'---'. $url .'---';
...
function sac_addData($sac_user_name, $sac_user_text, $sac_user_url) {
	...
	$sac_user_name = (empty($sac_user_name)) ? __('Anonymous', 'sac') : sanitize_text_field($sac_user_name);
...
function sac_cron_thirty_minutes($schedules) {
	$schedules['thirty_minutes'] = array('interval' => 1800, 'display' => __('Thirty minutes'));
	return $schedules;
}
...
function sac_cron_thirty_seconds($schedules) {
	$schedules['thirty_seconds'] = array('interval' => 30, 'display' => __('Thirty Seconds'));
	return $schedules;
}
...
function sac_cron_truncate_chats() {
	...
	$default_message = isset($sac_options['sac_default_message']) ? $sac_options['sac_default_message'] : __('Welcome to the Chat Forum', 'sac');
	$default_handle  = isset($sac_options['sac_default_handle'])  ? $sac_options['sac_default_handle']  : __('Simple Ajax Chat', 'sac');
	...
	$truncate_result = __('Successfully truncated SAC table.', 'sac');
	if ($truncate === false) $truncate_result = __('Unable to truncate SAC table.', 'sac');
	elseif ($truncate === 0) $truncate_result = __('Truncate not needed, zero rows affected.', 'sac');
	
	$insert_result = __('Successfully inserted default message.', 'sac');
	if ($insert === false) $insert_result = __('Unable to insert default message.', 'sac');
	elseif ($insert === 0) $insert_result = __('Default message not inserted.', 'sac');
...
function sac_plugin_action_links($links) {
	...
	$link = '<a href="'. $href .'" title="'. __('Visit SAC Settings', 'sac') .'">'. __('Settings', 'sac') .'</a>';
...
function add_sac_links($links, $file) {
	...
	if ($file == plugin_basename(__FILE__)) {
		...
		$href  = 'https://wordpress.org/support/view/plugin-reviews/'. basename(dirname(__FILE__)) .'?rate=5#postform';
		$title = __('Click Here to Rate and Review this Plugin on WordPress.org', 'sac');
		$text  = __('Rate this plugin&nbsp;&raquo;', 'sac');
