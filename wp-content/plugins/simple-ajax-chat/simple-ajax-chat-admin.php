...
function sac_default_options() {
	...
	return array(
		...
		'sac_default_handle'  => __('Simple Ajax Chat', 'sac'),
...
function sac_render_form() {
	...
	if (!empty($chats)) {
		...
		$chat_report .= sprintf(_n(' is %s', ' are %s', $count_chats, 'sac'), $count_chats);
		$chat_report .= sprintf(_n(' chat message (your default message)', ' chat messages', $count_chats, 'sac'), $count_chats);
	} else {
		$chat_report .= __(' are 0 chat messages. Please add at least one message via the chat box.', 'sac');
	} ?>
	...
	<style type="text/css">
		...
		#mm-chat-list .button-secondary { margin: 0 3px; }
	...
	<div id="mm-plugin-options" class="wrap">
		...
		<div id="mm-panel-toggle">
			<a href="<?php echo admin_url('options-general.php?page=simple_ajax_chat'); ?>"><?php _e('Toggle all panels', 'sac'); ?></a>
		</div>
			...
			<div class="meta-box-sortables ui-sortable">
				...
				<form method="post" action="options.php">
					...
					<div id="mm-panel-alert"<?php echo $display_alert; ?> class="postbox">
						<h2><?php _e('Simple Ajax Chat needs your support!', 'sac'); ?></h2>
						<div class="toggle">
					...
					<div id="mm-panel-primary" class="postbox">
						...
						<div class="toggle<?php if (!$sac_updated_options) { echo ' default-hidden'; } ?>">
							...
							<h3><?php _e('General options', 'sac'); ?></h3>
							...
							<h3><?php _e('Times and colors', 'sac'); ?></h3>
							<div class="mm-table-wrap">
								<table class="widefat mm-table">
									<tr>
							...
							<h3><?php _e('Appearance', 'sac'); ?></h3>
							<div class="mm-table-wrap">
								<table class="widefat mm-table">
									<tr>
										<th scope="row"><label class="description" for="sac_options[sac_custom_styles]"><?php _e('Custom CSS styles', 'sac'); ?></label></th>
										<td><textarea class="textarea large-text code" rows="10" cols="50" name="sac_options[sac_custom_styles]"><?php echo esc_textarea($sac_options['sac_custom_styles']); ?></textarea>
							...
							<h3><?php _e('Targeted loading', 'sac'); ?></h3>
							<div class="mm-table-wrap">
								<table class="widefat mm-table">
									<tr>
										<th scope="row"><label class="description" for="sac_options[sac_script_url]"><?php _e('Chat URL', 'sac'); ?></label></th>
										<td>
											<td><textarea class="textarea" rows="3" cols="50" name="sac_options[sac_script_url]"><?php echo $sac_options['sac_script_url']; ?></textarea>
											<div class="mm-item-caption"><?php _e('By default, the plugin includes its JavaScript on <em>every</em> page. To prevent this, and to include its JavaScript only on the chat page, enter the URL where it&rsquo;s displayed. Separate multiple URLs with a comma. Leave blank to disable.', 'sac'); ?></div>
										</td>
									</tr>
								</table>
							</div>
							<h3><?php _e('Custom content', 'sac'); ?></h3>
							<div class="mm-table-wrap">
								<table class="widefat mm-table">
									<tr>
				...
				<div id="mm-panel-tertiary" class="postbox">
					<h2><?php _e('Manage Chat Messages', 'sac'); ?></h2>
					...
					<div class="toggle<?php if (!$sac_updated_message) { echo ' default-hidden'; } ?>">
						...
						<div class="mm-table-wrap">
							...
							<?php } else {
									...
									foreach ($chats as $chat) {
										...
										<div><span><?php echo sprintf(__('Last Message: %s ago', 'sac'), sac_time_since($chat->time)); ?></span></div>
										<ul id="mm-chat-list">
										<?php } ?>
											...
											<li>
												<form method="get" action="options.php">
													...
													<span class="mm-chat-text">
														<input type="text" name="sac_text" value="<?php echo $chat->text; ?>" size="50" /> 
														<input type="hidden" name="sac_comment_id" value="<?php echo $chat->id; ?>" /> 
														<input type="submit" name="sac_delete" value="Delete" class="button button-secondary" /> 
														<input type="submit" name="sac_edit" value="Edit" class="button button-secondary" /> 
													</span>
												</form>
											</li>
						...
						<form method="get" action="options.php">
							<input type="submit" name="sac_truncate" class="button-secondary" id="mm_truncate_all" value="<?php _e('Delete all chat messages', 'sac'); ?>" />
						</form>
					</div>
				</div>

				<div id="mm-panel-quaternary" class="postbox">
					<h2><?php _e('Banned Phrases', 'sac'); ?></h2>
				...
				<div id="mm-restore-settings" class="postbox">
					<h2><?php _e('Restore Default Options', 'sac'); ?></h2>
					<?php if (isset($_GET["sac_restore_success"])) {
					...
					<div class="toggle<?php if (!$sac_restore_options) { echo ' default-hidden'; } ?>">
						...
						<hr />
						<p><strong><?php _e('Delete all plugin settings', 'sac'); ?></strong></p>
