		...
		if ($results) {
			...
			foreach($results as $r) {
				...
				if ($sac_first_time == true) {
					...
					$sac_lastout  = '<div id="sac-latest-message"><span>'. __('Latest Message:', 'sac') .'</span> ';
					$sac_lastout .= '<em id="responseTime">'. sprintf(__('%s ago', 'sac'), sac_time_since($chat_time)) .'</em></div>'. "\n";
					
				} 
				
				$sac_out  = '<li class="sac-chat-message sac-static sac-user-'. $name_class .'" data-time="'. date('Y-m-d,H:i:s', $chat_time) .'">'. "\n";
				$sac_out .= '<span title="'. sprintf(__('Posted %s ago', 'sac'), sac_time_since($chat_time)) .'">'. $url .' : </span> ';
			...
			$sac_output .= '<li>'. __('You need at least one entry in the chat forum!', 'sac') .'</li>';
