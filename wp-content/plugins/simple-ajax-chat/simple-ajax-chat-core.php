...
$sac_die = __('Please do not load this page directly. Thanks!', 'sac');
	...
	if (!empty($sac_referer) && $sac_match) {
		...
		if ($sac_no_js && $sac_verify) {
			...
			if ($sac_name && $sac_chat) {
				...
				do_action    ('sac_process_chat_action', $sac_name, $sac_chat, $sac_url);
				apply_filters('sac_process_chat_filter', $sac_name, $sac_chat, $sac_url);
			...
			if (!empty($sac_user_name) && !empty($sac_user_text) && $sacSendChat === 'yes') {
				...
				do_action    ('sac_process_chat_action', $sac_user_name, $sac_user_text, $sac_user_url);
				apply_filters('sac_process_chat_filter', $sac_user_name, $sac_user_text, $sac_user_url);
