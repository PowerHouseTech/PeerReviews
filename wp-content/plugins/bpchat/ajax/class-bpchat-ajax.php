...
class bpchat_Ajax {
	...
	public function chat_user_online($user_id, $time=5){
			global $wpdb;
			$sql = $wpdb->prepare( "
				SELECT u.user_login FROM $wpdb->users u WHERE u.ID = %d AND DATE_ADD( u.bpchat_last_activity, INTERVAL %d MINUTE ) >= UTC_TIMESTAMP()", $user_id, $time);
    ...
    public function load_bpc_window_function() {
    	...
    	$member_list = !empty($data['bpc_lg_member_list'])?$data['bpc_lg_member_list']:'CONTACTS';
		$members = !empty($data['bpc_lg_members'])?$data['bpc_lg_members']:'CONTACTS';
    ...
    public function load_friends_function() {
		...
		if($this->is_buddypress && $this->is_bpfriend){
			$FriendsSQL = $wpdb->get_results($wpdb->prepare("SELECT u.id AS id, u.display_name AS display_name FROM $wpdb->users u, {$wpdb->prefix}bp_friends bpf WHERE u.id NOT LIKE '%d' AND ((bpf.initiator_user_id = '%d' AND bpf.friend_user_id = u.id AND bpf.is_confirmed = 1) OR (bpf.initiator_user_id = u.id AND bpf.friend_user_id = '%d' AND bpf.is_confirmed = 1)) AND DATE_ADD( u.bpchat_last_activity, INTERVAL %d MINUTE ) >= UTC_TIMESTAMP() ORDER BY u.id LIMIT 50", $UserId, $UserId, $UserId, $time));

			$FriendsSQL1 = $wpdb->get_results($wpdb->prepare("SELECT u.id AS id, u.display_name AS display_name FROM $wpdb->users u, {$wpdb->prefix}bp_friends bpf WHERE u.id NOT LIKE '%d' AND ((bpf.initiator_user_id = '%d' AND bpf.friend_user_id = u.id AND bpf.is_confirmed = 1) OR (bpf.initiator_user_id = u.id AND bpf.friend_user_id = '%d' AND bpf.is_confirmed = 1)) AND DATE_ADD( u.bpchat_last_activity, INTERVAL %d MINUTE ) <= UTC_TIMESTAMP() ORDER BY u.id LIMIT 50", $UserId, $UserId, $UserId, $time));
		}else{
			$FriendsSQL = $wpdb->get_results($wpdb->prepare("SELECT id, display_name FROM $wpdb->users WHERE id NOT LIKE '%d' AND DATE_ADD( bpchat_last_activity, INTERVAL %d MINUTE ) >= UTC_TIMESTAMP() ORDER BY id LIMIT 50", $UserId, $time));
			$FriendsSQL1 = $wpdb->get_results($wpdb->prepare("SELECT id, display_name FROM $wpdb->users WHERE id NOT LIKE '%d' AND DATE_ADD( bpchat_last_activity, INTERVAL %d MINUTE ) <= UTC_TIMESTAMP() ORDER BY id LIMIT 50", $UserId, $time));
		}
