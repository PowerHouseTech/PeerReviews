...
class Theme_My_Login_User_Moderation extends Theme_My_Login_Abstract {
	...
	public function action_messages( &$theme_my_login ) {
		if ( isset( $_GET['pending'] ) ) {
			switch ( $_GET['pending'] ) {
				case 'activation' :
					$theme_my_login->errors->add( 'pending_activation', __( 'Your registration was successful. Please check your email and click on the link provided to activate your account.', 'theme-my-login' ), 'message' );
					break;
				case 'approval' :
			...
			switch ( $_GET['activation'] ) {
				case 'complete' :
					if ( class_exists( 'Theme_My_Login_Custom_Passwords' ) )
						$theme_my_login->errors->add( 'activation_complete', __( 'Your account has been activated and may now log in. Don\'t forget to update your profile!', 'theme-my-login' ), 'message' );
					else
