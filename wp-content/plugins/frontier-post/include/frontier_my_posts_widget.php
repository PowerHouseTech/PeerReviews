			...
			foreach ( $fp_wdata['presult'] as $q_post)
				...
				if ( $last_post != $q_post->post_id )
					...
					if ($tmp_post->post_status == "publish")
						echo '<a class="frontier-my-post-widget-link frontier-my-post-widget-status-publish" href="'.get_permalink($q_post->post_id).'">'.$q_post->post_id.'</a>';
					else
						echo '<div id="frontier-my-post-widget-status-'.$tmp_post->post_status.'">'.$q_post->post_id.'</div>';
