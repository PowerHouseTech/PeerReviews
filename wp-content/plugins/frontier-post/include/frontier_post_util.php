		...
		*/
		...
		switch ($input_type)
			...
			case "multi":
				...
				echo '</br><div class="frontier_helptext">'.__("Add tags", "frontier-post").'</div>';
				// echo '<script src="chosen.jquery.js"></script>';

				// echo frontier_post_tax_multi(array('first option', 'second option', 'third option'), array(1,2), 'mySelect', 'mySelect', '10');
				break;
...
Function frontier_post_tax_multi($tmp_cat_list, $tmp_selected, $tmp_name, $tmp_id, $tmp_size)
	{
	$tmp_html = '<select class="frontier_post_dropdown" data-placeholder="Add tags..." name="'.$tmp_name.'" id="'.$tmp_id.'" multiple="multiple" size="'.$tmp_size.'">';
